...
function formatUnit(itemRow) {
  if (!itemRow?.unidad_medida_nombre && !itemRow?.unidad_medida_simbolo) return "-";
  if (itemRow.unidad_medida_nombre && itemRow.unidad_medida_simbolo) {
    return `${itemRow.unidad_medida_nombre} (${itemRow.unidad_medida_simbolo})`;
  }
  return itemRow.unidad_medida_nombre || itemRow.unidad_medida_simbolo || "-";
}
...
export default function OrdenRequerimientoTable({
  ...
  onDeliver,
  onConfirmarTecnico,
  emptyMessage = "No hay ordenes de requerimiento registradas.",
}) {
  if (loading) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white px-6 py-12 text-center text-sm text-gray-500">
        Cargando ordenes de requerimiento...
      </div>
    );
  }
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span
                    className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${
                      STATUS_STYLES[orden.estado] || STATUS_STYLES.POR_REVISAR
                    }`}
                  >
                  ...
                  {orden.pendiente_confirmacion_tecnico && (
                    <span className="inline-flex rounded-full px-3 py-1 text-xs font-semibold bg-blue-100 text-[#1e3a8a]">
                      Pendiente de validacion del tecnico
                    </span>
                  )}
                ...
                <div className="mt-2 flex flex-wrap gap-4 text-sm text-gray-500">
                  ...
                  <span>Tecnico: {orden.tecnico_asignado_nombre || "Sin asignar"}</span>
            ...
            <div className="mt-4 overflow-hidden rounded-xl border border-gray-200">
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-600">Codigo</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {(orden.items || []).map((itemRow) => (
                    <tr key={itemRow.id}>
                      ...
                      <td className="px-4 py-3 text-gray-700">{formatQty(itemRow.cantidad)}</td>
                      <td className="px-4 py-3 text-gray-500">{formatUnit(itemRow)}</td>
                      ...
                      {canManageDetails && (
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {itemRow.puede_marcar_sin_stock && (
                              <button
                                type="button"
                                onClick={() => onMarkSinStockItem?.(orden, itemRow)}
