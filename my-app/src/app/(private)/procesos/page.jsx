...
import TareaPorEstandarizarModal from "@/components/procesos/TareaPorEstandarizarModal";
import TareaPorEstandarizarTable from "@/components/procesos/TareaPorEstandarizarTable";
import { itemAPI, tareaPorEstandarizarAPI } from "@/lib/api";
...
export default function ProcesosPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [tareasResponse, itemsResponse] = await Promise.all([
        tareaPorEstandarizarAPI.list(),
        itemAPI.list(),
      ]);

      setTareas(normalizeCollection(tareasResponse));
      setItems(normalizeCollection(itemsResponse));
    } catch (loadError) {
      setError(
        loadError?.response?.data?.detail ||
          "No se pudo cargar la vista de procesos."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleOpenDocumento = (tarea) => {
    router.push(`/procesos/${tarea.id}`);
  };
  ...
  const filteredTareas = useMemo(() => {
    ...
    return tareas.filter((tarea) => {
      const matchesCriticidad =
        criticidad === "TODOS" || tarea.nivel_criticidad === criticidad;
      ...
      return matchesCriticidad && matchesSearch;
    });
  }, [criticidad, search, tareas]);

  const stats = useMemo(() => {
    const total = tareas.length;
    const desarrolladas = tareas.filter((tarea) => tarea.desarrollado).length;
    const criticidadAlta = tareas.filter(
      (tarea) => tarea.nivel_criticidad === "ALTO"
    ).length;

    return {
      total,
      desarrolladas,
      pendientes: total - desarrolladas,
      criticidadAlta,
    };
  }, [tareas]);
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <h1 className="mt-4 text-3xl font-semibold tracking-tight text-slate-900">
                Estandarizacion de tareas operativas
              </h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
                Registra y consulta las tareas que deben documentarse dentro del flujo
                operativo de la plataforma, con su criticidad, area e item relacionado.
              </p>
            </div>
          ...
          <label className="block">
            ...
            <div className="relative mt-2">
              ...
              <input
                ...
                placeholder="Codigo, tarea, area o item"
      ...
      <TareaPorEstandarizarTable
        tareas={filteredTareas}
        loading={loading}
        onOpenDocumento={handleOpenDocumento}
      />

      <TareaPorEstandarizarModal
        open={openModal}
